...
/**
 draw a block with a border by drawing the block passed on top of a background block
...
void Block::drawWithBorder(fgcugl::Color borderColor, int borderSize)
{
	...
	// draw iner block
	Block foreground = Block(xpos + borderSize, ypos + borderSize, width - 2 * borderSize, height - 2 * borderSize, color);
	foreground.draw();
}