...
bool update(Ball& ball, Paddle& paddle, Block bricks[BRICK_ROWS][BRICK_COLUMNS], Walls walls, double lag, Direction nextD);
...
int main()
{
	...
	int padx = WINDOW_WIDTH / 2.0 - PADDLE_WIDTH / 2;
...
bool update(Ball& ball, Paddle& paddle, Block bricks[BRICK_ROWS][BRICK_COLUMNS], Walls walls, double lag, Direction next)
{
	...
	else
	{
		...
		int collision = ball.collisionCheck(walls.top);
			...
			// check for collision of ball with the bricks
